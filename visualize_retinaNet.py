...
saved_folder = 'RetinaNet_LostAndFound_Static_Results_1000_boxes'
...
num_forward = 10
top_uncertain_boxes = 1000

uncertainty_type = 'entropy' #'variance' # 'entropy'
...
for i in big_outlier_list:
	...
	#---------------------------------------------------------------------------------------------------------------------------
	# remove background anchor boxes by keeping largest score larger than 0.1
	mean_scores = np.mean(pred_all_scores, axis=0)
	max_mean_scores = np.max(mean_scores, axis=1)
	keep = max_mean_scores > 0.05
