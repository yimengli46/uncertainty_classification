...
saved_folder = 'trained_model/prop_cls_maskrcnn_coco/duq/{}'.format(rep_style)
...
def train_classifier(train_loader, classifier, optimizer):
    ...
    for i in range(len(train_loader)):
        ...
        with torch.no_grad():
            classifier.eval()
            classifier.update_embeddings(images, masks, labels.long())
...
for epoch in range(1, NUM_EPOCHS+1):
    ...
    test_loss = test_classifier(ds_val, classifier)
